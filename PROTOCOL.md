...
- **The build number leads the handoff.** When the work belongs to a numbered build series (Build 8.4, Build 11.6, …), that number is what Charles orients by — not the Brief ID, plan id, or project slug. So it appears in two fixed places, every time: as the first line of the Handoff Reference block (`**Build:** 11.6`), and as the opening words of the pasteable prompt (`Build 11.6 — execute Build Brief BB-YYYY-MM-DD-slug. …`). It also leads the plan's `plain_title` ("Build 11.6 — …"), per the naming rule above. All the other identifiers stay exactly as they are; the number sits in front of them. If a brief genuinely has no number in a series, the same first line reads `**Build:** unnumbered` rather than being dropped — Charles should never have to work out which it is. Assigning the number is the authoring chat's job, never Charles's. (Charles's instruction, 2026-09-20.)
...
- **Done receipt gate.** Marking a plan succeeded requires a done receipt (Session Log id + every acceptance check passed with evidence or handed off to a new plan); the database refuses otherwise.
...
- **A freshly deployed or newly added MCP tool is not callable in the session that deployed it.** The tool manifest is fixed at connector-handshake time — a server redeploy, a brand-new tool name, and even opening a fresh chat are not enough on their own; the connector itself must be reconnected (toggled off/on) after the deploy, and even then the *building* chat's own toolset stays frozen mid-session. Budget for a fresh, reconnected session to make the first live call on any newly added MCP tool, and verify at the DB/logic layer (e.g. Supabase `execute_sql`) as a fallback when a same-session call is unavoidable. (Lessons E-080, E-085, E-222, D-083, CB-062/063, A-041 — six independent rediscoveries since June.)
- **Stack-map upkeep.** cbrain holds the canonical stack-map (`get_entity('stack-map')`), which is the component map on the cbrain-ui Stack screen. Sessions never edit it directly and no longer file map suggestions — that standing rule (MAP-EDIT next moves) is retired. The Inspector compares the map to what shipped each week and files Punch List items for drift; the Repairer applies them. See "## The Punch List," below.
